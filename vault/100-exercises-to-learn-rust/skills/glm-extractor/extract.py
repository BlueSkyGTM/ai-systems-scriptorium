#!/usr/bin/env python3
"""100-exercises-to-learn-rust extractor — GLM runner for a book+exercises repo.

This repo pairs an mdBook narrative (`book/src/NN_topic/NN_section.md`) with
hands-on exercises (`exercises/NN_topic/NN_section/` — code only, NO README).
The problem statements live in the BOOK, not the exercise folders. So:

  single  -- one GLM call from a blob you build (inventory, book structure,
             curriculum map, antilibrary note).
  topics  -- fan out one GLM call per book topic: concatenate every *.md in
             book/src/<NN_topic>/ and summarize the topic (Rust concepts + seam
             relevance), assembled in topic order under a header.
...
SYSTEM = (
    "You are a curriculum extraction assistant for a Rust language-foundation "
    "module in an AI systems-engineering course. Rust is taught as the serving "
    "layer, CLI tooling, and concurrency/async substrate. Read raw book content "
    "and produce structured markdown in the exact format requested. Be precise "
    "and faithful; no opinions."
)
...
def do_topics(args):
    ...
    fails = sum(1 for s in ordered if "FAILED" in s or "NO BOOK" in s)
...
def main():
    ...
    pt.add_argument("--max-tokens", type=int, default=1200); pt.set_defaults(func=do_topics)
