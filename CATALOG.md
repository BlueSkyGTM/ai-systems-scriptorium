...
| **Just Python** | 🚧 In progress | [`library/in-progress/just-python`](library/in-progress/just-python/CONTEXT.md) | Applied Python (NumPy/Pandas/vectorization; 94% screen, gap-report row 1). **Modules 1–7 shipped** (M1 2026-06-20; M2–M5 2026-06-21: "NumPy in Depth", "Pandas for AI Pipelines", "Vectorization Discipline", "Python Idioms at Interview Speed", each 5 lessons + 4 exercises; M6 + M7 2026-06-21, the two portfolio modules — "Data Wrangling Artifact" (`wrangle.py`) and "Evaluation Engine" (`eval_engine.py`, per-class P/R/F1 in NumPy, no sklearn), each a runnable artifact + `pytest` smoke gate + skill write-up. Reviewed to STYLE + STANDARDS, `mdbook build` clean). M8 (Integrated Exam, the capstone) next. |
