...
The read-only ACL is the load-bearing line, not the Python. A kill switch the agent can `SET` is theater.
...
Sometimes the right response is not to kill the agent but to *contain* it — keep it running, sever its reach, and watch. At the kernel layer, eBPF and Cilium can rewrite a quarantined pod's network egress on the fly: the agent thinks it is talking to the production database; its packets are transparently rerouted to a forensic honeypot. The agent keeps acting, you keep observing, and nothing it does lands on a real system. It is a kill switch with a one-way mirror — you stop the harm without stopping the evidence. [verify: Cilium / eBPF — egress redirection / network policy at the kernel layer]
