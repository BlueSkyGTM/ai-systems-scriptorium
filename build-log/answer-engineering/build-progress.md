...
| M4 | Technical Screens | ✅ Shipped | 2026-06-21 | Authored last (after M8), resolving the deferral and completing the book 8/8. Overview + 4 lessons (narrating-the-screen, the-happy-path-and-the-edges, debugging-out-loud, the-clock-and-the-weak-screen-audit) + 4 exercises. The live-coding screen taught as the **communication layer** (clarify, narrate, debug out loud, test as you go), NOT LeetCode. **The deferral was resolved by reframing the worked tasks around the real, already-tested Sans Python portfolio code** (budget guard, relevance floor, content guardrail, sandbox timeout, tool path-jail, kill switch, drift monitor) so nothing is fabricated; each task traces to a cited real file. Ends in the weak-screen audit (the M2/M5 audit analogue). **Grows `check_prep.py` to v8 in place** (`--module 4` gates `coding-screen-log.md`; M4 is a SIBLING gate to M5-M8, not a prerequisite, so their gates stay byte-identical; included in `--module all`; 89/89 tests). Honest scope: M4 teaches the communication layer + AI-eng patterns, not a substitute for dedicated algorithm drilling (which M8 still says to source separately); NOT wired into `grade_dossier.py` (the coding screen stays outside the graded loop). Plan: `m4/PLAN.md`. |
...
M4 was authored LAST (after M8), on Ray's "go" to resolve the one deferral, and it completes the book
**8 of 8**. The original block (the vault has no live-coding-screen worked examples; building it as scoped
would mean fabricating coding problems) was removed by **reframing the worked tasks around the real,
already-tested Sans Python portfolio code**: a read-only Haiku surveyed `library/completed/sans-python`
and returned ~9 grounded coding tasks (budget guard, relevance floor, content guardrail, sandbox timeout,
tool path-jail, kill switch, drift monitor, eval gate), each tracing to a cited real file; the conductor
wrote the `coding-screen-tasks.md` ingredient + `00-overview` + SUMMARY; four Sonnet lesson-writers (each
reading the cited real file to ground its shown code) + one Sonnet exercises/validator worker authored the
rest. The module teaches the live-coding **communication layer** (clarify, narrate, debug out loud, test
as you go) on real AI-engineering patterns, not LeetCode, and keeps the honest line that dedicated
algorithm drilling is sourced separately (M8's stance preserved; M4 is NOT wired into `grade_dossier.py`).
VERIFY (conductor): caught + fixed two issues the workers missed — em-dashes inside illustrative code
comments (-> colons) and a fourth-wall leak where L4 used the ingredient's internal "Task F/Task G" labels
as section headings (-> plain titles). No fabrication: every shown pattern traces to a real portfolio file
(verified against `retriever.py`, `budget.py`, `guardrail.py`, `sandbox.py`, `tools/__init__.py`,
`killswitch.py`, `drift.py`). BUILD/TEST: `mdbook build` clean; `check_prep.py` v8 with 89/89 pytest (M4 a
sibling `--module 4` gate; M5-M8 byte-identical; `--module all` includes it); `calibrate.py` and
`grade_dossier.py` `--selftest` still green. `GATE-APPROVE-SHIP` cleared under the standing directive.
**Answer Engineering is COMPLETE: 8 of 8 modules shipped.**
