...
build. **Modules 1-5 shipped 2026-06-21** (`GATE-APPROVE-SHIP`): M1 "The Build" (`HARDWARE.md` +
...
baseline M7 tunes against), and M5 "The Routing Layer" (`ROUTING.md` + `check_routing.py` + `route.py`,
which wraps `ollama_client.py` as the local arm of a local-vs-cloud router). See
`build-log/local-metal/build-progress.md`. Next: M6 (Wire to Claude Code) via the pipeline.
