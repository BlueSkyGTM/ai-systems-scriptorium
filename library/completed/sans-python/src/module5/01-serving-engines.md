...
The training rigs, the CUDA kernels, the optimizer math — that depth is real, and it lives in the antilibrary, on the *Avec Python* path. It is not on the road to this job. What is on the road is the serving layer: the program that holds the model weights in GPU memory, accepts requests over HTTP, batches them, and streams tokens back. That program is an **inference serving engine**, and choosing one is the first real decision of the module.
