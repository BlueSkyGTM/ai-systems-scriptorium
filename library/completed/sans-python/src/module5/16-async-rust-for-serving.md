...
The `?` operator from lesson 15 works inside async functions exactly as it does in sync ones; a failed `.await` short-circuits and returns the error. Async did not change how Rust handles failure; it only changed when the work runs.
...
<div class="claude-handoff" data-exercise="exercises/module5/16-async-rust-for-serving/">
