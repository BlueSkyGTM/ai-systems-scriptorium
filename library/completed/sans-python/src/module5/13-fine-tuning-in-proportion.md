...
- The deep, from-scratch training build is Path-A MLE work, left to a focused companion; the AI Platform Engineer's job is to make the fine-tune-vs-RAG-vs-prompt decision, specify the approach, and speak to it precisely; not to hand-build the training loop.
