...
- The deep, from-scratch training build is Path-A MLE work, named in the antilibrary as a future *Avec Python*; the AI Platform Engineer's job is to make the fine-tune-vs-RAG-vs-prompt decision, specify the approach, and speak to it precisely — not to hand-build the training loop.
