...
**Build it in Claude Code** — build an eval framework that traces a RAG pipeline, scores outputs on the RAG triad with an LLM-as-judge, runs statistical correction with confidence intervals, and produces a pass/block report. Open the repo and run the exercise for this lesson.
