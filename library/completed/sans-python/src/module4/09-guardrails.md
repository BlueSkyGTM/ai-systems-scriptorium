...
**Constitutional AI** trains the model itself to reason about principles rather than match a list of banned strings. Anthropic's January 2026 constitution gives the model a written charter and a **four-tier priority hierarchy** the model resolves when principles conflict:

1. **Safety and supporting human oversight** — the top tier. Nothing overrides it.
2. **Ethics** — broad ethical conduct, honesty, avoiding harm.
3. **Anthropic's guidelines** — the model developer's rules for the model.
4. **Helpfulness** — be useful to the user.

The order is the point. When helpfulness pulls against safety, safety wins by construction, not by a patch. Separate from this ordering, the constitution splits its rules into two hardness classes that matter operationally:
...
This split is what you operate against: you get knobs, but the knobs stop at the prohibitions, and that boundary is not yours to move. The honest limit of reason-based alignment is that it relies on the model *generalizing* its principles to situations nobody anticipated — which is more robust than a keyword list and still not a guarantee.

**Llama Guard** is the other shape: a separate classifier model that screens content at runtime, on the way in and on the way out. Meta's Llama Guard 3 and 4 take a message — a user prompt or a model response — and label it safe or unsafe against the MLCommons hazard taxonomy (Llama Guard 4 is a 12B multimodal classifier covering categories S1–S14, including S14 code-interpreter abuse). It runs as input/output classification around your agent:
