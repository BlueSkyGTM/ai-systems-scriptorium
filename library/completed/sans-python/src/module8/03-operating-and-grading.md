...
You will not find a lesson teaching these from scratch here. The exam expects you to apply them when the architecture you replicate demands one; to know that a multi-tenant RAG version leans on sharding, that an eval-gated CI/CD version leans on caching the golden-set results, that the inbox is an async queue with a human on the far end. Knowing the word is the floor; knowing which seam needs it is the job.
