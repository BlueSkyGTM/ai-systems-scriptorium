...
The environment and the transformer-forward NLP that everything LLM-related assumes. Two chapters:

1. **Setup & Tooling** — the day-one, four-layer install stack. The universal root: every hands-on lesson
   in the course depends on it.
2. **NLP: Transformer-Forward** — from **attention** forward. How transformers work is seam knowledge for
   every engineer who designs prompts and evals. The pre-transformer half (classical NLP, RNN/LSTM, seq2seq)
   is in the [Antilibrary](../antilibrary.md).
