...
``python -m pytest tests/`` run on the standard library alone. You reach for
this only when you set ANTHROPIC_API_KEY and want a live model driving the loop.

It implements the same ``respond(messages) -> ModelResponse`` interface the mock
does, so ``agent.run_agent`` does not change. That is the portable seam: the
loop, the tools, the budget, the kill switch, and the verify gate are all
model-agnostic; only this adapter knows about a vendor.
"""
...
# [verify: pricing — platform.claude.com/docs/en/pricing — $5/$25 per 1M (Opus 4.8)]
